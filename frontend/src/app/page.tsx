  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <Link href="/recipe/upload" className="bg-green-600 text-white px-5 py-2.5 rounded-full font-semibold hover:bg-green-700 transition shadow-sm">
          + Share Recipe
        </Link>
      </div>
      ...
      {recipes.length === 0 ? (
        <div className="text-center py-20 bg-gray-50 rounded-2xl border border-dashed border-gray-300">
          ...
          <Link href="/recipe/upload" className="text-green-600 font-semibold hover:underline">Upload a recipe</Link>
        </div>
      ) : (
          ...
          {recipes.map((recipe: any) => (
            <div key={recipe._id} className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition">
              {recipe.images && recipe.images.length > 0 ? (
                ...
                <div className="w-full h-48 bg-emerald-50 flex items-center justify-center text-4xl">🍳</div>
              )}
              <div className="p-6">
                ...
                <div className="flex items-center justify-between border-t border-gray-100 pt-4">
                  <div className="flex items-center gap-4">
                    ...
                    <button className="flex items-center gap-1 text-gray-500 hover:text-green-600 transition font-medium text-sm">
                      🔖 Save
                    </button>
                  </div>
                  <button className="bg-green-50 text-green-700 px-4 py-2 rounded-xl text-sm font-bold hover:bg-green-100 transition">
                    + Add to Planner
                  </button>
                </div>
              </div>
